  ...
  <transition name="modal-fade">
    <div style="background-color: rgba(0, 0, 0, 0.3);" class="fixed left-0 top-0 right-0 bottom-0 flex justify-center items-center">
      <div class="overflow-x-auto w-1/3 flex flex-col" role="dialog" aria-labelledby="modalTitle" aria-describedby="modalDescription">
        <form class="bg-white m-3 flex flex-col justify-center items-center rounded" @submit.prevent="submitTask()">
          <h1 class="text-3xl text-gray-700 mt-8" style="font-family: 'Roboto Medium'">Add new task</h1>
          ...
          <input v-model="dateEnd" required type="date" class="w-1/2 text-xl py-2 px-4 mt-4 text-gray-600 rounded bg-blue-50">
...
export default {
  ...
  methods: {
    ...
    submitTask(){
      ...
      formData.append('date_start', today);
      ...
      axios.post( 'http://127.0.0.1:8000/api/tasks/createTask',
          formData,
          { headers: headers }
      ).then(() => {
        alert('Task was success added')
        this.name = ''
        this.dateEnd = ''
      })
      .catch((e) => {
